...
m = re.search("(openshift.example.com .*)", open("/etc/hosts").read(), re.M)
DOMAINS = ["%s 60 IN A %s" % (domain, IP) for domain in m.group(1).split(" ")]

os.system("iptables -w -t nat -I PREROUTING -i tun0 -d %s -p udp --dport 53 -j REDIRECT --to-port 153" % NSIP)

resolver = TimeoutInterceptResolver(NSIP, 53, "60s", DOMAINS, [], [])
